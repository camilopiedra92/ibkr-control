...
- **Test infra world-class — PR-A: ✅ MERGEADO a `main`** (PR #15, merge commit `33495ab`, el 2026-06-11). **Track de tooling PARALELO al programa SaaS — NO bloquea SP2.** Reemplazó el provisioning de tests "contenedor Postgres nuevo + `alembic upgrade head` POR TEST" (fixture `_migrated_app_db`, ~1.7s/test en los 118 endpoint tests; el 28% de los tests se comía el 64% del wall-clock) por **template database migrado UNA vez por worker + clon barato por test** (`CREATE DATABASE ... TEMPLATE`, copia filesystem ~100ms) + `pytest-xdist -n auto` por defecto (provisioning síncrono psycopg2+alembic → sin fixtures async session-scoped; clones namespaced por `PYTEST_XDIST_WORKER`; `DROP ... WITH (FORCE)` en teardown). **Suite 311.70s → ~33s (~9x)**, 428 tests, CI verde en el runner de 4 cores. **Cero cambio semántico** (los endpoint tests ya corrían como `app_rls`). Guards de paridad nuevos (anti-regresión a futuro): clone-fidelity (`relforcerowsecurity` en las 17 tablas org-scoped + `app_rls` rolsuper/rolbypassrls=false sobre clon fresco — espeja el boot guard de PR #7) + fail-closed (app_rls sin `org_context` → 0 filas, default-deny). **Decisiones clave (D1-D7):** una sola fuente de schema = migraciones, NO `create_all` (elimina la divergencia: hoy parte de la suite corría con RLS apagado contra un schema paralelo); `app_rls` default fail-closed, `owner` excepción nombrada; costo por-test **O(1) respecto al largo de la cadena de migraciones** (escala a largo plazo — el clon copia el resultado, no re-corre migraciones). Spec: `docs/specs/2026-06-11-test-infra-worldclass-design.md` (D1-D7 + rollout PR-A/B/C) · Plan PR-A: `docs/plans/2026-06-11-test-infra-pr-a-template-clone.md`. **PENDIENTE (próximos PRs del track, NO bloquean SP2):** **PR-B** = converger el model world (305 tests `db_session`/`create_all` → clon del mismo template, **sigue owner**, cero cambio semántico, borra el parche manual de grants/función conftest 145-156); **PR-C** = flip del rol del model world owner→`app_rls` (RLS enforced, aislado para que cualquier rojo sea inequívocamente el flip, no la fuente del schema). El model world quedó INTACTO en PR-A (el template vive en otra DB del mismo contenedor, no colisiona con el `create_all`). **Lección:** un "boot smoke OK" / tests verdes bajo un rol que no es el de prod NO prueban RLS — los guards de fidelidad lockean que el clon preserva el contexto de seguridad (refuerza [[rls-runtime-vs-test-parity]]).
