...
- **⏯ SIGUIENTE (próxima sesión) — W3 (Restatement log, PR-3):** W2 implementado (PR pending merge); el securities master ya es definitivo. Arrancar **W3 (restatement log, PR-3)** sobre `main` post-merge — branch `saas/w3-restatement-log` desde `main`, mismo spec Tier 1 (`docs/specs/2026-06-10-tier1-worldclass-model-design.md`), misma política re-baseline pre-deploy (T1-D14). **Nota de implementación:** el hook `DO UPDATE` de `_ensure_instruments` (anti-churn material-change) debe refactorizarse en W3 para **capturar old→new** y emitir al restatement log (hoy solo aplica el cambio; el linaje del cambio es justamente lo que W3 construye — ver notas del spec). Luego SP2 (Authorization) queda detrás de los PRs restantes de Tier 1 — ver el bullet "SIGUIENTE — SP2" abajo.
