...
| **12 items de deuda conocida D1-D12** documentados en polish backlog (D12 = wizard TRM backfill silencioso, agregado 2026-05-25) | Aceptados como V1 | No bloquean — la mayoría tienen mitigation o son features (fail-loud audit, etc.) |
...
- **D12** Wizard `setup._trm_backfill_background` fire-and-forget silencioso — mitigation operacional: Settings → Refresh manual cubre el caso con feedback SSE completo
...
- **TRM fetch — bulk upsert + SSE contract drift (2026-05-25)** — primer disparo real del TRM post-deploy (cron 00:30 UTC + manual desde Settings + wizard `step2/save`) reveló que el job venía fallando silenciosamente desde el merge. `trm_days` quedó en 0 con 4 rows `failed` en `ingest_log`. Dos bugs independientes que se compusieron para producir la sensación "no hace nada":
  ...
  - **Pendiente (no bloquea Phase 3, deuda separada):** `api/setup.py::_trm_backfill_background` que dispara el wizard post-`step2/save` sigue siendo fire-and-forget (FastAPI `BackgroundTasks` con `logger.exception` en el except). Si Socrata o el persister fallan, queda solo en logs + `ingest_log` sin feedback al usuario en el wizard — el "Setup completado!" aparece igual aunque el TRM nunca se haya bajado. Para feedback real habría que integrar al patrón SSE existente: registrar un job en `JobTracker` antes del `background.add_task`, devolver el `job_id` en la respuesta de `step2/save`, y que el frontend del wizard consuma `/api/ingest/stream/{job_id}` (Phase 5/polish, no bloquea). Mientras tanto la pantalla Settings → Refresh manual ya cubre el caso de "verificar TRM manualmente" con feedback completo.
