...
**Phase 2.x + 2.7 + 2.8 + 2.9 (lot `asset_class` cleanup) TODOS completos y mergeados a `main` (`8197c5d`). Phase ≤2.8 pusheadas a `origin` (tags `v0.2.0`→`v0.2.6` @ `b83a9fa`). Phase 2.9 (tag `v0.2.7-lot-asset-class`) mergeada LOCAL, PENDIENTE push a `origin` (red caída en la sesión 2026-06-02). Próximo: Phase 3.**
...
> ⚠ **Push a `origin` — PARCIALMENTE PENDIENTE (2026-06-02):** los 7 tags `v0.2.0`→`v0.2.6` + `main` hasta `b83a9fa` SÍ están en remote (verificado con `git ls-remote --tags origin`). **Phase 2.9 (`8197c5d` + tag `v0.2.7-lot-asset-class`) quedó SIN pushear** — el `git pull` se colgó en red durante la sesión. Pendiente cuando haya conexión: `git push origin main && git push origin v0.2.7-lot-asset-class`. Repo `github.com/owner/ibkr-control`.
