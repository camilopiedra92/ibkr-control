...
- **⏯ EN CURSO — Account-multihome (branch `saas/account-multihome`, spec + plan listos, implementación NO arrancada):** decisión usuario 2026-06-10 — invertir "cuenta broker single-org por diseño" → **tenancy canónica per-org (patrón Plaid/Sharesight)**: la misma cuenta IBKR puede existir en N orgs, universos aislados (trade-off de duplicación/divergencia de copias aceptado explícitamente; `access_grants`/participations intactos — SP2 igual). Supersede H2 (`AccountClaimedError` se desmantela entero: el conflicto cross-org deja de existir y `_ensure_accounts` se simplifica a ON CONFLICT + re-select org-scoped). Spec: `docs/specs/2026-06-10-account-multihome-design.md` (M1–M7) · Plan: `docs/plans/2026-06-10-account-multihome.md` (4 tasks). **Próxima sesión: ejecutar el plan con `superpowers:subagent-driven-development` sobre este branch** (baseline 357 tests, head Alembic `544a0b2c362c`). Va ANTES de SP2 (SP2 se diseña sobre la semántica de tenancy definitiva).
- **SIGUIENTE — SP2 (Authorization):** enforcement de los `access_grants` (acceso cross-org del contador), `require_*_scope`, ReBAC. Depende de SP1. SP1 dejó el dato del grant *shaped* (tabla `access_grants` + policy RLS especial); SP2 construye el *enforcement* (qué org puede hacer switch a qué org). Arrancar tras el merge de SP1 + sus follow-ups (hardening + rls-runtime-wiring + db-hardening): `superpowers:brainstorming` → spec `docs/specs/YYYY-MM-DD-sp2-authorization-design.md` → `writing-plans` → `subagent-driven-development`. Branch `saas/sp2-authorization` desde `main` (post-merge de SP1 + follow-ups).
