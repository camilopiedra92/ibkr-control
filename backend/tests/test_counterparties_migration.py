...
async def test_downgrade_reverts_cleanly():
    with PostgresContainer("postgres:16-alpine", driver="psycopg2") as pg:
        ...
        async with eng.begin() as conn:
            ...
            src_acct = await conn.scalar(text(
                "SELECT src_account_id FROM transfers WHERE transaction_id='T-FOP-1'"))
            assert src_acct is not None, "src_account_id should be non-null after downgrade"
            restored = await conn.scalar(text(
                "SELECT ibkr_account_id FROM accounts WHERE id=:a"), {"a": src_acct})
