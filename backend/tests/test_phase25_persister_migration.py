...
async def test_phase25_rev1_renames_observed_counters(alembic_db_session: AsyncSession):
    ...
    result = await alembic_db_session.execute(
        text(
    ...
    expected = {
        "n_observed_trades",
        "n_observed_lots_closed",
        "n_observed_open_lots",
        "n_observed_cash_tx",
        "n_observed_dividends",
        "n_observed_transfers",
    }
...
async def test_phase25_rev2_adds_unique_constraints(alembic_db_session):
    ...
    result = await alembic_db_session.execute(
        text("SELECT conname FROM pg_constraint WHERE conname = ANY(:names)").bindparams(
...
async def test_phase25_rev2_open_position_lots_natural_key_enforced(alembic_db_session):
    ...
    await alembic_db_session.execute(
        text("""
