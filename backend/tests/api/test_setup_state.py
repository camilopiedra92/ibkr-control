...
async def _connection_rows(owner_engine) -> list[dict]:
...
async def test_step1_save_creates_connection_active(
    ...
    rows = await _connection_rows(owner_engine)
    assert len(rows) == 1
...
async def test_step1_save_idempotent_updates_first_and_rotates(
    ...
    assert r.status_code == 200, r.text
    before = await _connection_rows(owner_engine)
    assert len(before) == 1
    conn_id = before[0]["id"]
    ...
    after = await _connection_rows(owner_engine)
