...
async def test_step3_save_new_accounts_happy_path(client: AsyncClient, auth_headers: dict):
    ...
    try:
        async with Session() as s:
            acc = await s.scalar(select(Account).where(Account.ibkr_account_id == "U88888888"))
            assert acc is not None
            parts = (
                await s.scalars(select(Participation).where(Participation.account_id == acc.id))
            ).all()
            assert len(parts) == 1
            assert parts[0].pct == Decimal("1.0000")
    finally:
...
async def test_step3_save_new_accounts_idor_rejects_id_not_in_upload(
    ...
    with U88888888 (stash), then tries to save U99999001 (not in their stash).
    Must 400 and create neither the account nor a participation.
    """
    from sqlalchemy import select
    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine
    ...
    try:
        async with Session() as s:
            ...
            assert acc is None
            assert (await s.scalars(select(Participation))).all() == []
    finally:
        await engine.dispose()


_XML_OWNER_ACCT = b"""<?xml version="1.0"?>
<FlexQueryResponse>
  <FlexStatements>
    <FlexStatement accountId="U99999001" fromDate="20240101" toDate="20241231">
