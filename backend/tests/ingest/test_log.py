...
@pytest.mark.asyncio
async def test_log_creates_running_row_on_enter(db_session: AsyncSession, sample_org, sample_user):
    async with ingest_log_entry(
        db_session, "flex", sample_user.id, sample_org.id, "cron"
    ) as log_id:
        result = await db_session.execute(
            text(
                "SELECT status, job_kind, user_id, organization_id, trigger "
...
@pytest.mark.asyncio
async def test_log_finishes_ok_on_normal_exit(db_session: AsyncSession, sample_org, sample_user):
    async with ingest_log_entry(
        db_session, "trm", sample_user.id, sample_org.id, "manual"
    ) as log_id:
        pass
    result = await db_session.execute(
...
@pytest.mark.asyncio
async def test_log_finishes_failed_on_exception(db_session: AsyncSession, sample_org, sample_user):
    with pytest.raises(ValueError, match="boom"):
        async with ingest_log_entry(
            db_session, "flex", sample_user.id, sample_org.id, "wizard"
        ) as log_id:
...
@pytest.mark.asyncio
async def test_log_items_processed_settable(db_session: AsyncSession, sample_org, sample_user):
