...
@pytest.mark.asyncio
async def test_ensure_accounts_cross_org_collision_raises_account_claimed(
    db_session: AsyncSession, sample_org
):
    """_ensure_accounts converts the global ibkr_account_id unique violation into a
    clean AccountClaimedError (not a raw IntegrityError), and the SAVEPOINT leaves
    the outer session usable afterward (no aborted-transaction state).

    Org A owns the account. Owner/db_session bypasses RLS, so to reproduce the
    RLS-blinded case (where org B cannot see org A's row and therefore tries to
    INSERT) we blind only the initial existence probe, exercising the REAL INSERT
    collision on the global unique constraint.
    """
    from sqlalchemy import select

    from ibkr_control.db.models.accounts import Account
    from ibkr_control.db.models.organizations import Organization
    from ibkr_control.ingest.flex.persister import AccountClaimedError, _ensure_accounts

    shared_id = "U-SHARED-CLAIM"

    org_b = Organization(type="personal", name="Org B Household")
    db_session.add(org_b)
    db_session.add(
        Account(organization_id=sample_org.id, ibkr_account_id=shared_id, currency="USD")
    )
    await db_session.commit()
    await db_session.refresh(org_b)

    # Blind the existence probe (first .scalars call) so the helper treats the
    # already-claimed account as missing and attempts the colliding INSERT.
    orig_scalars = db_session.scalars
    state = {"blinded": False}

    async def _blinded_scalars(stmt, *a, **kw):
        if not state["blinded"]:
            ...
            return _Empty()
        return await orig_scalars(stmt, *a, **kw)

    db_session.scalars = _blinded_scalars
    try:
        with pytest.raises(AccountClaimedError):
            await _ensure_accounts(db_session, [shared_id], organization_id=org_b.id)
    finally:
        db_session.scalars = orig_scalars

    # Generic message: leaks neither the account id nor the owning org.
    msg = str(AccountClaimedError())
    assert shared_id not in msg
    assert str(sample_org.id) not in msg

    # Session still usable: a fresh query succeeds, and org A's row is intact
    # (org B inserted nothing).
    acc = await db_session.scalar(select(Account).where(Account.ibkr_account_id == shared_id))
    assert acc is not None
    assert acc.organization_id == sample_org.id


@pytest.mark.asyncio
async def test_ensure_accounts_reraises_unrelated_integrity_error(
    db_session: AsyncSession, sample_org
):
    """Only the ibkr_account_id unique is converted. A DIFFERENT IntegrityError
    (here: a NOT NULL violation on organization_id) re-raises unchanged so
    unrelated DB errors are never masked as AccountClaimedError."""
    from sqlalchemy.exc import IntegrityError

    from ibkr_control.ingest.flex.persister import AccountClaimedError, _ensure_accounts

    with pytest.raises(IntegrityError):
        # organization_id=None violates the NOT NULL on accounts.organization_id —
        # a different constraint than ibkr_account_id unique → must NOT be masked.
        await _ensure_accounts(db_session, ["U-NEW-NN"], organization_id=None)

    # Sanity: it was not converted to the domain error.
    assert AccountClaimedError is not IntegrityError
