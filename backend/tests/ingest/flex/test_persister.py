...
@pytest.mark.asyncio
async def test_persist_rolls_back_on_partial_failure(
    db_session: AsyncSession, sample_user, sample_account
):
    """Si falla un INSERT a mitad (dup transaction_id), NADA queda en DB."""
    parsed = _make_parsed(account_id=sample_account.ibkr_account_id, n_trades=2)
    ...
    with pytest.raises(IntegrityError):
        await persist(
            ...
            parsed=parsed,
            user_id=sample_user.id,
            xml_bytes=b"<xml>partial-fail</xml>",
            source="manual_upload",
        )
    await db_session.rollback()

    # No debe haber quedado el FlexImport huerfano con trades
    fi_partial = await db_session.scalar(
        select(FlexImport)
        .where(FlexImport.source == "manual_upload")
        .order_by(FlexImport.id.desc())
        .limit(1)
    )
    if fi_partial:
        n_for_this = await db_session.scalar(
            select(func.count(Trade.id)).where(Trade.flex_import_id == fi_partial.id)
        )
        assert n_for_this == 0
