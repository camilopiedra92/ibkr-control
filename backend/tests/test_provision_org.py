...
@pytest.mark.asyncio
async def test_provision_org_hashes_password(db_session):
    ...
    user = await db_session.scalar(select(User).where(User.id == result.user_id))
...
@pytest.mark.asyncio
async def test_provision_org_returns_correct_ids(db_session):
    ...
    party = await db_session.scalar(select(Party).where(Party.id == result.party_id))
