...
**⚠ A3 AMENDED #3 2026-05-25 during Task 13 smoke test (closed_lots):** original natural key for `closed_lots` was `(transaction_id)` alone — collided against real IBKR data where multiple `<Lot levelOfDetail="CLOSED_LOT">` rows share the same `transactionID`. IBKR uses `transactionID` as a TRADE EXECUTION ID, not a row unique ID; when a close trade closes partial fractions of an open_lot across MULTIPLE separate sell executions over time, each execution emits its own `<Lot>` row but they all share the originating close trade's `transactionID`. Example verified in fixture 2025: ICSH (Joint-Retirement) transactionID=30666937825 emits 7 rows across 7 different dates (Jan 3 / Jan 21 ×2 / Jan 27 ×2 / Feb 5 / Feb 24) closing one open_lot opened 2024-12-30, each with distinct `dateTime`, `qty`, `cost`, `fifoPnlRealized`. Without discrimination, only the first row persisted → lost 6 close events + ~$8.75 realized PnL. Across the full 2025 XML this lost 31 closed_lot rows out of 146 (~21% data loss). Extended natural key to `(transaction_id, close_datetime, qty)`. Required:
- New parser field on `ParsedClosedLot.close_datetime: datetime` (parsed from `<Lot dateTime="YYYYMMDD;HHMMSS">`)
- New column `close_datetime TIMESTAMP NOT NULL` on `ClosedLot` model (qty already present as a column)
- New Alembic Revision 5: schema prep — add column NULLABLE + drop old UNIQUE
- Backfill script `backend/scripts/backfill_closed_lots.py` — replays xml_bytes (A0!) for each flex_import, DELETEs + re-inserts closed_lots with the new column. **No user re-upload required**: A0 was designed exactly for this case
- New Alembic Revision 6: promote `close_datetime` NOT NULL + add new UNIQUE constraint
- Persister update: pass `(transaction_id, close_datetime, qty)` in conflict_cols
- Detected during Task 13 manual smoke test 2026-05-25 — verified against real ICSH 2025 data showing 6 of 7 close events lost. Same pattern class as #1 and #2 (IBKR-emitted natural discriminator missing from spec).
