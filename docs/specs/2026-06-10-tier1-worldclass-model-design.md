...
| T1-D14 | Rollout PR-2 | **Wipe & reload** de tablas de hechos (precedente Phase 2.9/SP1: NOT NULL sin server_default exige tablas vacías; dev descartable, pre-deploy). PR-1 en cambio hace **migración data-preserving** (las `flex_credentials` existentes migran a `connections` + detail — es barato y correcto). |
...
- Hechos: columna `instrument_id` BigInt FK instruments `RESTRICT` — NOT NULL en `trades`, `closed_lots`, `open_position_lots`, `change_in_dividend_accruals`, `open_dividend_accruals`; nullable en `cash_transactions`; `transfers` según CR-1. Se **agrega** index `(account_id, instrument_id)` (hot path del FIFO Phase 3) y se **conserva** `(account_id, symbol)` (los consumidores actuales consultan por symbol; remover índices se decide con evidencia de uso, no especulando — criterio db-hardening).
...
|---|---|---|---|
| CR-1 | Grep de `conid` por tag contra los 3 fixtures reales (2024/2025/2026) | PR-2, antes de lockear NOT NULL | Presencia 100% de conid en Trade/Lot/OpenPosition/accruals; decidir transfers y cash por evidencia |
