...
- **W4 — State machine por conexión** _(→ SP7)_. Plaid modela el Item con estados user-facing: `healthy / degraded / reauth_required / disconnected`. El health endpoint actual (R4) es telemetría de *corridas* (`ingest_log`), no estado de la *conexión*. Cuando el token Flex expire, el usuario debe ver "tu conexión necesita re-autenticación" con CTA — no un banner genérico "ingest failed hace 48h". Mecánica: columna `status` + transiciones en la tabla `connections` de W1.
