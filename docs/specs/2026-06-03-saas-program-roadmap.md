...
- **SP3 (AuthN & onboarding) — dirección pre-decidida del provisioning UX (2026-06-12):** el registro **NO** crea org automáticamente (patrón GitHub-namespace **rechazado**: rompe invites — el contador/segundo miembro que se registra invitado no debe crear org propia → orgs basura que contaminan billing SP6 y el selector; acopla identidad con tenancy, que SP1 separó a propósito). Se va por **onboarding gate explícito post-signup** (patrón Slack/Linear/WorkOS): `register → verify → /onboarding (crear org [nombre + personal|firm] | aceptar invite) → wizard → app`. El estado "user sin membership" es **transitorio por diseño** (el 403 `NO_ORG_MEMBERSHIP` default-deny de `api/_context.py` es correcto y se queda); `POST /api/organizations` envuelve el mismo servicio de dominio que `provision_org` (org → membership owner → party atómico, una sola fuente de verdad; el CLI queda para ops); guard en el layout `(app)` redirige sin-membership → `/onboarding` (hoy el login hace `router.push("/dashboard")` ciego). La decisión sobrevive al fork fastapi-users vs IdP: si el IdP trae Organizations nativas cambia quién emite identidad, no el gate ni el servicio. El brainstorming de SP3 parte de acá (resuelve IdP, MFA, invites, multi-party, selector multi-org D-CONV-3, sandbox W8).
...
> **Tier 1 (W1-W4) COMPLETO 2026-06-11.** W1 (connections) + W4 (connection state machine) → PR W1; W2 (securities master) → PR W2; W3 (restatement log) → PR W3. Las decisiones de modelo caras de retrofitear quedaron tomadas antes de su SP/Phase consumidor. W5-W10 quedan mapeados a sus SPs (ver Tier 2 / Tier 3 abajo y la columna W# de la tabla de SPs).
